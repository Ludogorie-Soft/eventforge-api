...
//    @Override
//    public List<OneTimeEventResponse> filterEventsByCriteria(CriteriaFilterRequest request) {
//        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
//        CriteriaQuery<Event> query = cb.createQuery(Event.class);
//        Root<Event> root = query.from(Event.class);
//        List<Predicate> predicates = new ArrayList<>();
...
//            } else {
//                predicates.add(cb.isFalse(root.get("isOnline")));
//            }
//        }
//
//        if (request.getOrganisationName() != null) {
...
//        }
//
//        if (request.getRangeFrom() != null) {
//            predicates.add(cb.greaterThanOrEqualTo(root.get("startsAt"), request.getRangeFrom()));
//        }
//        if (request.getRangeTo() != null) {
//            predicates.add(cb.lessThanOrEqualTo(root.get("endsAt"), request.getRangeTo()));
//        }
//        if (request.getStartsAt() != null) {
//            predicates.add(cb.equal(root.get("startsAt"), request.getStartsAt()));
//        }
//        if (request.getEndsAt() != null) {
//            predicates.add(cb.equal(root.get("endsAt"), request.getEndsAt()));
//        }
//
//
//        if (request.getIsOneTime()) {
...
//        query.where(predicates.toArray(new Predicate[0]));
//
//        return entityManager.createQuery(query).getResultList().stream().map(event -> responseFactory.buildOneTimeEventResponse(event)).toList();
//    }
